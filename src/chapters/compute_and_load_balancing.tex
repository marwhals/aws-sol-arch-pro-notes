...
\begin{itemize}
    \item DNS Layer - Route 53
    \item Web Layer - CLB, ALB, NLB, API Gateway, Elastic IP
    \item Compute Layer - EC2, ASG, Lambda, ECS, Fargate, Batch, EMR
    \item CDN Layer - CloudFront
    \item Caching / Session Layer - ElastiCache, DAX, DynamoDB, RDS
    \item Database Layer - RDS, Aurora, DynamoDB, ElastiSearch, S3, Redshift
    \item Decoupling Orchestration Layer - SQS, SNS, Kinesis, Amazon MQ, Step Functions
    \item Storage Layer - EBS, EFS, Instance Store
    \item Static Assets Layer (storage) - S3, Glacier
\end{itemize}
...
\begin{itemize}
    \item R: applications that need a long of RAM - in-memory caches
    \item C: applications that need good CPU - compute / databases
    \item M: Applications that are balanced (think "medium") - general / web app
    \item I: Applications that need good local I/O (instance storage) - databases
    \item G: Applications that need a GPU - video rendering / machine learniing
    \item T2 / T3: burstable instances (up to a capacity)
    \item T2 / T3 - unlimited: unlimited burst
\end{itemize}
...
\begin{itemize}
    \item Control the EC2 instance placement strategy using placement groups
    \item Group strategies
    \item Cluster - cluster instances into a low-latency group in a single Availability Zone
    \item Spread - spreads instances across underlying hardware (max 7 instances per group per AZ) - critical applications
    \item Partition - spreads instances across many different partitions (which rely on different sets of racks) within an AZ. Scales to 100s of EC2 instances per group (Hadoop, Cassandra, Kafka)
    \item You can move an instance into or out of a placement group
    \item You first need to stop it
    \item You then need to use the CLI (modify-instance placement)
    \item You can then start your instances
\end{itemize}

Placement Groups Cluster
%TODO add diagram -- Same Rack, Same AZ ----- Placement Group Cluster. Low latency 10gbps network
\begin{itemize}
    \item Pros: Great network (10 Gbps bandwidth between instances with Enhanced Networking enabled - recommended)
    \item Cons: If the rack fails, all instances fails at the same time.
    \item Use case:
    \item Big Data job that needs to complete fast
    \item Applications that needs extremely low latency and high network throughput
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Pros
        \item Can span across Availability Zones (AZ)
        \item Reduced risk of simultaneous failure
        \item EC2 Instances are on different physical hardware
    \end{itemize}
    \item
    \begin{itemize}
        \item Cons
        \item Limited to 7 instances per AZ per placement group
        \item Use case:
        \item Application that need to maximise high availability
        \item Critical Applications where each instance must be isolated from failure from each other
    \end{itemize}
\end{itemize}

Placement Groups Partition
\begin{itemize}
    \item Up to 7 partitions per AZ
    \item Up to 100s of EC2 instances
    \item The instances in a partition do not share racks with the instances in the other partitions
    \item A partition failure can affect many EC2 instances but won't affect other partitions
    \item EC2 instances get access to the partition information as metadata
    \item Use cases: HDFS, HBase, Cassandra, Kafka
\end{itemize}

EC2 Instance Launch Types
\begin{itemize}
    \item On Demand Instances: Short workload, prefictable pricing, reliable
    \item Spot Instance: short workloads, for cheap, can lose instances (not reliable)
    ...
    \item Highest to lowest discount: All upfront payment, partial upfront payment, no upfront
    \item Dedicated Instances: No other customers will share your hardware
    \item Dedicated Hosts: book an entire physical server, control instance placement
    \item Great for software licenses that operate at the core, or CPU socket level
    \item Can define host affinity so that instance reboots are kept on the same host
\end{itemize}

EC2 Graviton
\begin{itemize}
    \item AWS Graviton Processors deliver the best price performance
    \item Supports many Linux OS, Amazon Linux 2, RedHat, SUSE, Ubuntu
    \item Not available for windows instances
    \item Graviton2 - 40\% better price performance over comparable 5th generation x86 based instances
    \item Graviton3 - Up to 3x better performance compared to Graviton2
    \item Use cases: app servers, microservices, HPC, CPU-based ML, video encoding, gaming, in-memory caches,.....
\end{itemize}

EC2 included metrics
\begin{itemize}
    \item CPU: CPU Utilisation + Credit Usage / Balance
    \item Network: Network In / Out
    \item Status Check
    \item Instance status = check the ECM VM
    \item System status = check the undelying hardware
    \item Disk: Read / Write for Ops / Bytes (only for instance store)
    \item RAM is not included in the AWS EC2 metrics
\end{itemize}

EC2 Instance Recovery
\begin{itemize}
    \item Status Check
    \item Instance Status = check the EC2 VM
    \item System status = check the underlying hardware
%TODO add diagram
    \item Recovery: Same Private, Public, Elastic IP, metadata, placement group
\end{itemize}


\section{High Performance Computing (HPC)}
\begin{itemize}
    \item Cloud is good for HPC
    \item Can create a very high number of resources in no time.
    \item Can speed up time to results by adding more resources
    \item You can pay only for the systems you have used
    ...
    \item Services which help HPC?
\end{itemize}

Data Management \% Transfer
\begin{itemize}
    \item Aws Direct Connect
    \item Move GBs of data to the cloud over a private secure network
    \item Snowball
    \item Move PB of data to the cloud
    \item AWS DataSync
    \item Move large amount of data between on-premise and S3, EFS, FSx for Windows
\end{itemize}

Computer and Networking
\begin{itemize}
    EC2 Instances:
    \item CPU optimised, GPU optimised
    \item Spot Instances / Spot Fleets for cost savings + Auto Scaling
    \item EC2 Place Groups: Cluster for good network performance
%TODO add digram ---- repeat ---- cluster group in same rack and same AZ for low latency
    \item EC2 Enhanced Networking (SR-IOV)
    \item Higher Bandwidth, higher PPS (packet per second), lower latency
    \item Option 1: Elastic Network Adapted (ENA) up to 100 Gbps
    \item Option 2: Intel 82599 up to 10Gbps - Legacy
    \item Elastic Fabric Adapter (EFA)
    \item Improved ENA for HPC only works for Linux
    \item Great for inter-node communications, tightly coupled workloads
    \item Leverages Message Passing interface (MPI) standard
    \item Bypasses the underlying Linux OS to provide low-latency reliable transport
\end{itemize}

Storage
\begin{itemize}
    \item Instance-attached storage
    \item EBS: scale up to 256000 IOPS with io2 Block Express
    \item Instance Store: Scal to million of IOPS, linkedin to EC2 instance, low latency.
\end{itemize}

\begin{itemize}
    \item Network Storage
    \item Amazon S3: large blob, not a file system
    \item Amazon EFS: scale IOPS based on total size or use provisioned IOPS
    \item Amazon FSx for Lustre:
    \item HPC optimised distributed file system, millions of IOPS
    \item Backed by S3
\end{itemize}
...
\begin{itemize}
    \item AWS Batch
    \item AWS Batch supports multi-node parallel jobs which enables you to run single jobs that span multiple EC2 instances
    \item Easily schedule jobs and launch EC2 instances accordingly
    \item AWS ParallelCluster
    \item Open source cluster management tool to deploy HPC on AWS
    \item Configure text files
    \item Automate creation of VPC, Subnet, cluster type and instance types
\end{itemize}
...
\begin{itemize}
    ...
    \item Most simple and easy to setup
    \item Example: I want the average ASG CPU to stay at aroudnd 40\%
    \item Simple / Step Scaling
    \item When a CloudWatch alarm is triggered (example CPU  > 70\%) then add 2 units
    \item When a CloudWatch alarm is triggered (example CPU < 30\%) then remove 1
    \item Scheduled Actions
    \item Anticipate a scaling based on known usage patterns
    \item Example: Increase the min capacity to 10 at 5pm on Fridays
\end{itemize}

Auto Scaling Groups - Predictive Scaling
\begin{itemize}
    \item Predictive Scaling: continuously forecast load and schedule scaling ahead
\end{itemize}
%TODO add useful diagrams

Good metrics to scale on
\begin{itemize}
    \item CPUUtilisation: Average CPU utilisation across your instances
    \item RequestCountPerTarget: to make sure the number of requests per EC2 instances is stable
    \item Average Network In / Out (if you're application is network bound)
    \item Any custom metric (that you push using CloudWatch)
%TODO add diagram
\end{itemize}

Auto Scaling - Good to know
\begin{itemize}
    \item Spot fleet support (mix on Spot and On-Demand instances)
    \item Lifecycle Hooks
    \item Perform actions before an instance is in service, or before it is terminated
    \item Example: Cleanup, log extraction, special health checks
    \item To upgrade an AMI, must update the launch configuration / template
    \item Then terminate instances manually (CloudFormation can help)
    \item Or use EC2 instance refresh for Auto Scaling
\end{itemize}

Auto Scaling - Instance Refresh
\begin{itemize}
    \item Goal: Update launch template and then re-creating all EC2 instances
    \item For this we can use the native feature of Instance Refresh
    \item Setting of minimum healthy percentage
    \item Specify warm-up time (how long until the instance is ready to use)
    %TODO add diagram
\end{itemize}

Auto Scaling - Scaling Processes
\begin{itemize}
    \item Launch: Add a new EC2 to the group, increasing the capacity
    \item Terminate: Remove an EC2 instance from the group, decreasing the capacity
    \item HealthCheck: Check the health of the instances
    \item ReplaceUnhealthy: Terminate unhealthy instances and re-create them
    \item AZRebalanced: Balancer the number of EC2 instances across AZ
    \item Alarm Notification: Accept notification from CloudWatch
    \item SchedulesActions: Performs scheduled actions that you create
    \item AddToLoadBalancer: Add instances to the load balancer or target group.
    \item InstanceRefresh: Perform an instance refresh
    ----> These processes can be suspended
\end{itemize}

Auto Scaling - Health Checks
\begin{itemize}
    \item Health Checks available:
    \item EC2 Status Checks
    \item ELB Health Checks (HTTP)
    \item Customer Health Checks - send instances health to an ASG using AWS CLI or AWS SDK (set-instnance-health)
    \item ASG will launch a new instance after terminating an unhealthy one
    \item Make sure the health check is simple and check ths correct thing
\end{itemize}
...
\begin{itemize}
    ...
    \item The hourly spot price varies based on offer and capacity
    \item If the current spot price > your max price you can choose to stop or terminate your instance with a two minutes grace period
    \item Used for batch jobs, data analysis or workloads that are resilient to failures.
